<?php
/*
	Plugin Name: Neo gallery
	Plugin URI: http://www.webpsilon.com/wordpress-plugins/
	Description: Flash Neo gallery. Very easy to use. Load your images(jpg, gif, png) in: wp-content/plugins/neo-gallery/images/
	Version: 1.1
	Author: Webpsilon
	Author URI: http://www.webpsilon.com/wordpress-plugins
*/	
$contador=0;
function neo_gallery_head() {
	
	$site_url = get_option( 'siteurl' );
			echo '<script src="' . $site_url . '/wp-content/plugins/neo-gallery/Scripts/swfobject_modified.js" type="text/javascript"></script>';
			
}
function neo_gallery($content){
	$content = preg_replace_callback("/\[neo_gallery ([^]]*)\/\]/i", "neo_gallery_render", $content);
	return $content;
	
}

function neo_gallery_render($tag_string){
$contador=rand(9, 9999999);
	$site_url = get_option( 'siteurl' );
global $wpdb; 	
$table_name = $wpdb->prefix . "neo_gallery";	
...
if(count($myrows)<1) $myrows = $wpdb->get_results( "SELECT * FROM $table_name;" );
  ...
  <param name="movie" value="'.$site_url.'/wp-content/plugins/neo-gallery/neo-gallery.swf" />
  ...
  <param name="expressinstall" value="'.$site_url.'/wp-content/plugins/neo-gallery/Scripts/expressInstall.swf" />
  <!-- Next object tag is for non-IE browsers. So hide it from IE using IECC. -->
  <!--[if !IE]>-->
  <object type="application/x-shockwave-flash" data="'.$site_url.'/wp-content/plugins/neo-gallery/neo-gallery.swf" width="'.$width.'" height="'.$height.'">
    <!--<![endif]-->
    <param name="quality" value="high" />
    <param name="wmode" value="transparent" />
	  <param name="scale" value="exactfit" />
...
function neo_gallery_instala(){
   ...
   $sql = " CREATE TABLE $table_name(
	...
	$wpdb->query($sql);
	$sql = "INSERT INTO $table_name (folder, timeback,  onover, links, width, height, title) VALUES ('wp-content/plugins/neo-gallery/images/', '5', '1', '', '100%', '250px', 'Webpsilon WP Plugins');";
	$wpdb->query($sql);
}
function neo_gallery_desinstala(){
	global $wpdb; 
	$table_name = $wpdb->prefix . "neo_gallery";
	$sql = "DROP TABLE $table_name";
	$wpdb->query($sql);
}	
function neo_gallery_panel(){
	global $wpdb; 
	$table_name = $wpdb->prefix . "neo_gallery";	
...
if(empty($re))
{
  $sql = " CREATE TABLE $table_name(
	...
	$wpdb->query($sql);
		...
		$sql = "INSERT INTO $table_name (folder, timeback,  onover, links, width, height, title) VALUES ('wp-content/plugins/neo-gallery/images/', '5', '1', '', '100%', '250px', 'Webpsilon Plugins');";
	$wpdb->query($sql);
	}
...
function neo_gallery_add_menu(){	
	if (function_exists('add_options_page')) {
		//add_menu_page
		add_options_page('neo_gallery', 'Neo gallery', 8, basename(__FILE__), 'neo_gallery_panel');
	}
}
if (function_exists('add_action')) {
	add_action('admin_menu', 'neo_gallery_add_menu'); 
}
add_action('wp_head', 'neo_gallery_head');
add_filter('the_content', 'neo_gallery');
add_action('activate_neo_gallery/neo_gallery.php','neo_gallery_instala');
add_action('deactivate_neo_gallery/neo_gallery.php', 'neo_gallery_desinstala');
?>